...
%Soit $n\ge 2$.Montrer que $P(X)=X^n+5X^{n-1}+3$ est irréductible sur $\Z[X]$.
%\end{exo}
...
Comme $s_0,s_1\in \Z$ on a par récurrence que $s_n\in \Z$ pour tout eniter $n$. On passe la relation modulo $5$ ce qui donne
$$s_{n+1}=s_n-s_{n-1} $$
...
%$$P=(b_0+b_1X+\dots+b_mX^m)(c_0+\dots+c_{n-m}X^{n-m})$$
...
%$$b_ic_0+\underbrace{b_{i-1}c_1+\dots+b_0c_i}_{\equiv 0[3]}\not\equiv 0[3] $$
%Comme $P$ a ses coefficient nuls pour $1\le i \le n-2$, cela assure que $i\ge n-1$ et donc que $\deg(_0+\dots+c_{n-m}X^{n-m})\le 1$. \\
%Comme la décomposition est non triviale, on doit avoir
%$$X+1|P $$ ou encore
%$$X-1|P $$
%Mais on vérifie que $P(1)=9\ne 0$ et que $P(-1)=(-1)^n+5(-1)^{n-1}+3\ne 0$. C'est l'absurdité recherchée.
%\end{sol}
