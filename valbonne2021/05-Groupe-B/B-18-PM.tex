...
$$f(-1)+f(x)f(0)=-1 $$ Si $f(0)\ne 0$, on peut diviser et donc $f$ est constante. C'est absurde car aucune fonction constante n'est solution. Donc $f(0)=0$ ce qui donne
$$f(-1)=-1 $$
Pour $x\ne 0$ on pose $y=\frac{1}{x}$ ce qui donne
...
\end{dfn}
\begin{rem}
Cela s'écrit que $a\ne a'\Rightarrow f(a)\ne f(a')$. Néanmoins, pour montrer qu'une fonction est injective, on raisonne par contraposée. On montre en fait que
...
$$f(f(a)f(c))=f(a)c $$
Et donc on a
$$f(a)b=f(a)c $$ et comme $f(a)\ne 0$ on a $b= c$. Donc $f$ est injective.
...
\\
\textbf{Attention:} Ici, $c$ est positif donc on a la périodicité seulement pour $x\ge a$.
...
Alors par ce qui précède, on se donne $a>0$ te $T=b-a$ tels que $f$ est $T-$périodique pour $x\ge a$. Pour forcer des simplifications, on part de la relation
$$f(x)f(y)=2f(x+yf(x)) $$ et on pose $y_0=\frac{T}{f(x)}$ pour $x\ge a$. Cela donne donc
$$f(x)f(y_0)=2f(x+T)=2f(x)\Rightarrow f(y_0)=2 $$
On a donc trouvé un antécédant de $2$. On pose alors $x=y_0$ ce qui donne
$$f(y)=f(y_0+2y) $$
Ainsi, par ce qui précède, on a montré que pour tout $y>0$, $f$ est $(y_0+y)$ périodique pour $x\ge y$.
\\
